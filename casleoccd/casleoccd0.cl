...
# Tarea para procesar imágenes directas con CCDs del CASLEO
...
bool redfoc{yes, prompt='¿Se usó reductor focal? (solo JS-Roper)'}
...
bool procesa{no, prompt='¿Procesa todas las imágenes?'}
...
bool cosmicos{no, prompt='¿Corrige por rayos cósmicos (lacos-im)?'}
...
bool flatHWP{no, prompt='¿Flats por cada ángulo HWP?'}
...
# Parámetros para el JS+Roper-2048 (directo)
if(chip=='JS-Roper'){
# escala del telescopio en arcsec/mm
escala=11.3
# tamaño del pixel en micrones
pix=13.5
npix=2048
x1=1
y1=1
...
# Parámetros para el JS+Tek-1024 (CasPol)
if(chip=='JS-Tek'){
# escala del telescopio en arcsec/mm
escala=11.3
# tamaño del pixel en micrones
...
# Parámetros para el HSH+STL-1001E
if(chip=='HSH-STL'){
# escala del telescopio en arcsec/mm
escala=22.6
# tamaño del pixel en micrones
...
# Regiones para Roper s/RF ó STL-1001E
else{
  xtr1=x1
  xtr2=npix/binxy
  ytr1=y1
...
# Pone los valores de EPADU y RDNOISE en los parámetros y grafica
...
# Procesa imágenes
if(prc){
  ccdproc(imag=lima, outp="", ccdtype="object", noproc-)
}

# Corrige rayos cósmicos
if(csmk){
  print("Corrigiendo rayos cósmicos ...")
  list=lm
  while (fscan (list, nomim) != EOF) {
   print(nomim)
   kim=strlen(nomim)
